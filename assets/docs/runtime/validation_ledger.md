...
| 25 | Patient timeline generation, rendering, and persistence | Sessions / timeline | PARTIAL | Two current independent qwen3.5:2b runs persisted empty timelines despite three dated source facts; the exact qwen3.5:9b control produced three source-backed events with month/day precision, rendered provenance, and reload persistence. | The 2B route does not meet grounded-output acceptance; earlier unknown and structured-parser failures remain separate. [Repeatability report](../../QA/timeline-local-model-repeatability-20260924/report.md) |
