# Error Handling
Last updated: 2026-06-03

## Scope
Apply these rules across backend and frontend code.

## Exception Boundaries
- Handle failures at clear boundaries such as API handlers, integration clients, and background runners.
- Catch specific exception types where possible.
- Avoid broad `except Exception` unless converting to a safe boundary response.
- Never silently swallow exceptions.

## Required Failure Classes
Every external interaction must consider:
- invalid input or schema mismatch
- timeout
- network or dependency unavailability
- malformed upstream responses
- missing files or resources

Retries are allowed only for transient failures and must define:
- max attempts
- backoff policy
- hard stop condition

## Timeouts And Cancellation
- Use explicit timeouts for all I/O and network calls.
- In async or threaded workflows, propagate cancellation signals and stop safely.
- Long-running jobs must be cancellable at safe checkpoints.

## User-safe Failures
- User-facing errors must be concise and non-technical.
- Never expose stack traces, secrets, access keys, or internal infrastructure details.
- Prefer degraded or partial output over total failure when safe.

## Logging And Diagnostics
Log enough context to debug:
- error class
- operation or context name
- request or job correlation ID when available

Never log:
- secrets or tokens
- raw provider keys
- sensitive personal data

## Resource Cleanup
- Clean up files and temp artifacts on success and failure.
- Clean up DB and network handles.
- Clean up thread or async control state.
- Prefer context managers and explicit finalization paths.

## Test Expectations
Tests should cover:
- validation errors
- dependency failures
- timeout and retry behavior
- cancellation behavior for background jobs
- safe user-visible error messaging

Expected result: failure must not crash the app or leak internal details.
