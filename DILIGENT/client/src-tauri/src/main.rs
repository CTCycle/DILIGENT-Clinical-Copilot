...
fn sync_workspace_payload(workspace_root: &Path, runtime_root: &Path) -> Result<(), String> {
    ...
    fs::create_dir_all(runtime_root.join("DILIGENT").join("resources").join("logs")).map_err(
        |error| {
            format!(
