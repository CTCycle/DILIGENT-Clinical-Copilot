...
class DataSerializer:
    ...
    def sanitize_livertox_records(self, records: list[dict[str, Any]]) -> pd.DataFrame:
        ...
        required_columns = ["nbk_id", "drug_name", "excerpt"]
        if df.empty:
    ...
    # -----------------------------------------------------------------------------
    def save_drugs_catalog(self, records: pd.DataFrame) -> None:
        frame = records.copy()
        frame = frame.reindex(columns=DRUGS_CATALOG_COLUMNS)
        frame = frame.where(pd.notnull(frame), None)
        database.save_into_database(frame, "DRUGS_CATALOG")
