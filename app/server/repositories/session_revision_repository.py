...
class SessionRevisionRepository:
    ...
    def _manual_edit_row(self, row: ClinicalSessionVersion) -> dict[str, Any]:
        ...
        return {
            ...
            "actor_display_name": repository_values.normalize_string(
                audit.get("actor_display_name")
            ),
            ...
            "reviewer_note": repository_values.normalize_string(
                audit.get("reviewer_note")
            ),
    ...
    def delete_session_revisions(self, session_id: int) -> bool:
        ...
        with self.session_factory() as db_session:
            ...
            version_ids = list(
                db_session.execute(
                    select(ClinicalSessionVersion.id).where(version_scope)
                ).scalars()
            )
            run_scope = (
            ...
            run_ids = list(
                db_session.execute(
                    select(ClinicalSessionRevisionRun.id).where(run_scope)
                ).scalars()
            )
            if version_ids:
                db_session.execute(
                    delete(ClinicalSessionRevisionReview).where(
                        ClinicalSessionRevisionReview.revision_version_id.in_(
                            version_ids
                        )
                    )
                )
    ...
    def update_current_report_text_with_manual_audit(
        ...
        *,
        ...
        with self.session_factory() as db_session:
            ...
            payload = (
                parse_session_result_payload(result_row.payload_json)
                if result_row
                else {}
            )
            payload = payload or {}
            previous_report = (
                repository_values.normalize_string(payload.get("report")) or ""
            )
            ...
            if result_row is None:
                db_session.add(
                    ClinicalSessionResult(
                        ...
                        payload_json=serialized_payload or "{}",
                    )
                )
            else:
        ...
        with self.session_factory() as db_session:
            ...
            if row is None:
                ...
                existing_configuration = (
                    parse_session_result_payload(row.configuration_json) or {}
                )
                if (
                ...
                row.configuration_json = serialize_json_payload(
                ...
                row.actor_display_name = repository_values.normalize_string(
                    initiated_by
                )
        ...
        with self.session_factory() as db_session:
            row = db_session.execute(
                ...
                .where(
                    ...
                    *(
                        [
                            ClinicalSessionRevisionStep.attempt_number
                            == int(attempt_number)
                        ]
                ...
                if isinstance(entries, list):
                    for index, entry in enumerate(entries):
                        if isinstance(entry, dict):
                            created.extend(
                                self.persist_revision_artifact(
    ...
    def list_revision_artifacts_for_version(
        self, *, revision_version_id: int, include_superseded: bool = False
    ) -> list[dict[str, Any]]:
        with self.session_factory() as db_session:
            statement = select(ClinicalSessionRevisionArtifact).where(
            ...
            rows = (
                db_session.execute(
                    statement.order_by(
                        ClinicalSessionRevisionArtifact.created_at.asc(),
            ...
            return [
                serialize_revision_artifact_row(row)
                for row in rows
                if include_superseded or row.status != "superseded"
            ]
    ...
    def persist_revision_entities(
        ...
        *,
        ...
        structured_case = result_payload.get("structured_case")
        if isinstance(structured_case, dict):
            for section_name, entity_type, schema_name in (
                ("therapy_drugs", "drug", REVISION_DRUG_SCHEMA_NAME),
                ("anamnesis_drugs", "drug", REVISION_DRUG_SCHEMA_NAME),
                ("anamnesis_diseases", "disease", REVISION_DISEASE_SCHEMA_NAME),
            ):
                ...
                for index, entry in enumerate(entries):
                    if not isinstance(entry, dict):
                        continue
                    name = repository_values.normalize_string(
                        entry.get("name") or entry.get("drug_name")
                    )
                    row = create_revision_entity_row(
                        revision_version_id=int(revision_version_id),
                        ...
                        requires_human_review=not bool(name),
                        payload=entry,
    ...
    def list_revision_entities_for_version(
        ...
        with self.session_factory() as db_session:
            ...
            return [
                serialize_revision_entity_row(row)
                for row in rows
                if include_superseded or row.status != "superseded"
            ]
    ...
    def record_revision_review_action(
        ...
        *,
        ...
        if clinical_review_status not in {
            "under_review",
            "approved_by_human",
            "rejected_by_human",
        }:
        ...
        with self.session_factory() as db_session:
            ...
            row = ClinicalSessionRevisionReview(
                revision_version_id=int(version.id),
                session_id=int(version.session_id) if version.session_id else None,
                clinical_review_status=clinical_review_status,
                reviewer_note=repository_values.normalize_string(reviewer_note),
                reviewed_by=reviewer,
                actor_id=None,
                actor_display_name=reviewer,
                actor_source="manual_entry" if reviewer else "unknown",
                actor_confidence="unverified",
                metadata_json=serialize_json_payload(metadata or {}),
                reviewed_at=datetime.now(UTC),
            )
            db_session.add(row)
            version.clinical_review_status = clinical_review_status
            version.version_status = {
                "approved_by_human": "human_approved",
                "rejected_by_human": "human_rejected",
            }.get(clinical_review_status, "requires_human_review")
            db_session.commit()
            db_session.refresh(row)
