...
def _load_catalog() -> dict[str, Any]:
    ...
    if not isinstance(catalog, dict) or not isinstance(
        catalog.get("policy_version"), str
    ):
...
def _values(
    ...
    mode = (
        "reasoning_enabled"
        if reasoning_level is not ReasoningLevel.OFF
        else "reasoning_disabled"
    )
    ...
    *,
    ...
    if purpose in _REVISION_PURPOSES:
        return (
            ...
            if user_reasoning_level is ReasoningLevel.HIGH
            else ReasoningLevel.LOW
        )
    if purpose is GenerationPurpose.TIMELINE_EXTRACTION:
        if (
            timeline_complexity == "complex"
            and user_reasoning_level is ReasoningLevel.HIGH
        ):
...
def resolve_generation_policy(
    *,
    ...
    exact = _CATALOG.get("exact_models", {}).get(
    ...
    if (
        ...
        and normalized_model.startswith("gpt-5")
    ):
