...
class RagDocumentReference(BaseModel):
    ...
    # -------------------------------------------------------------------------
    @model_validator(mode="after")
    def validate_location_ranges(self) -> "RagDocumentReference":
        if (
...
class DrugClinicalAssessment(BaseModel):
    ...
    rxnav_rxcui: str | None = Field(default=None, max_length=32)
    rxnav_validation_status: str | None = Field(default=None, max_length=64)
    match_status: str | None = Field(default=None, max_length=32)
    match_confidence: float | None = Field(default=None, ge=0.0, le=1.0)
    match_reason: str | None = Field(default=None, max_length=100)
    match_notes: list[str] = Field(default_factory=list)
    evidence_quality: str | None = Field(default=None, max_length=100)
