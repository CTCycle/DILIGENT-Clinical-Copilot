...
def test_start_interval_and_prompt_are_deterministic() -> None:
    ...
    suspension = service.evaluate_suspension(
        DrugEntry(
            name="Drug", therapy_start_status=True, therapy_start_date="2025-04-01"
        ),
        visit_date=date(2025, 4, 14),
    )
    ...
    assert (
        service.format_suspension_prompt(suspension)
...
def test_format_visit_date_anchor_covers_missing_and_present_values() -> None:
    ...
    assert (
        ExposureTimelineService.format_visit_date_anchor(date(2025, 4, 14))
        == "2025-04-14"
    )
