...
def test_detects_keywords_and_stopwords_from_catalog() -> None:
    ...
    matches = detect_seed_matches("Patient uses Bactrim tablets and mg dosage.", catalog)
    assert "bactrim" in matches["matched_keywords"]
    assert "tablets" in matches["matched_stopwords"] or "mg" in matches["matched_stopwords"]
