...
def test_course_scoring_uses_target_drug_withdrawal_date() -> None:
    ...
    assert component.score == 2
    assert component.evidence_date == "2025-02-15"


###############################################################################
def test_course_scoring_requires_meaningful_marker_fall() -> None:
    estimator = RucamScoreEstimator()
    component = estimator.score_course(
        injury_type="hepatocellular",
        lab_timeline=PatientLabTimeline(
    ...
    assert component.status == "not_assessable"
    assert component.score == 0
    assert "50%" in (component.rationale or "")


###############################################################################
def test_cholestatic_course_scoring_uses_cholestatic_markers() -> None:
    estimator = RucamScoreEstimator()
    component = estimator.score_course(
        injury_type="cholestatic",
        lab_timeline=PatientLabTimeline(
            entries=[
                ClinicalLabEntry(
                    marker_name="ALT",
    ...
    assert component.status == "scored"
    assert component.score == 2
    assert "600" in (component.evidence or "")
