...
###############################################################################
def test_effective_rag_documents_path_reads_v2_manifest_source(
    monkeypatch,
    tmp_path: Path,
) -> None:
    service = object.__new__(DataInspectionService)
    manifest_path = tmp_path / DataInspectionService.RAG_MANIFEST_FILE_NAME
    manifest_path.write_text(
        '{"source": {"documents_path": "C:\\\\external\\\\rag"}}',
        encoding="utf-8",
    )
    monkeypatch.setattr(service, "rag_manifest_path", lambda: manifest_path)
    monkeypatch.setattr(
        service,
        "load_runtime_config",
        lambda: {"rag": {"documents_path": r"C:\\configured\\rag"}},
    )
...
def test_write_rag_manifest_persists_latest_successful_source(
    ...
    monkeypatch.setattr(service, "rag_manifest_path", lambda: manifest_path)
    monkeypatch.setattr(
        "services.inspection.update_config.RAG_ACTIVE_GENERATION_POINTER_PATH",
        tmp_path / "active_generation.json",
    )
