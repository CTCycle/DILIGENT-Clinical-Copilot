...
class _FakeSerializer:
    ...
    def replace_manifest_entries(
        self, manifest: CatalogManifest, manifest_hash: str, source_path: str
    ) -> int:
        self.replaced.append(manifest.manifest)
        return len(manifest.entries)

    def record_seed_success(
        self, manifest: str, version: int, hash: str, source_path: str, entry_count: int
    ) -> None:
        self.success.add((manifest, hash))

    def record_seed_failure(
        self, manifest: str, version: int, hash: str, source_path: str, error: str
    ) -> None:
...
def test_seeder_skips_existing_hash(monkeypatch) -> None:  # type: ignore[no-untyped-def]
    ...
    monkeypatch.setattr(
        seeder_module, "iter_catalog_manifest_paths", lambda: [Path("a.json")]
    )
    monkeypatch.setattr(seeder_module, "compute_manifest_hash", lambda _path: "h1")
    monkeypatch.setattr(
        seeder_module, "load_catalog_manifest", lambda _path: _manifest("a")
    )
    result = ReferenceCatalogSeeder(serializer).seed_missing_or_changed_manifests(
        force=False
    )
