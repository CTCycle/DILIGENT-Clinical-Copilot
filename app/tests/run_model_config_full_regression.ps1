...
try {
    ...
    $runDbPath = Join-Path $tempRoot "codex-modelconfig-full.$logStamp.db"
    $pytestCacheDir = Join-Path $tempRoot "codex-modelconfig-full-pytest-cache.$logStamp"
    ...
    $backendOutLog = Join-Path $tempRoot "codex-modelconfig-full-backend.$logStamp.out.log"
    $backendErrLog = Join-Path $tempRoot "codex-modelconfig-full-backend.$logStamp.err.log"
    $frontendOutLog = Join-Path $tempRoot "codex-modelconfig-full-frontend.$logStamp.out.log"
    $frontendErrLog = Join-Path $tempRoot "codex-modelconfig-full-frontend.$logStamp.err.log"
    ...
    Write-Host "[STEP] Running full model-config API + app-flow e2e suite..."
    ...
    if (
        ...
        & $backendPython -m pytest $e2eModelConfigPath $e2eAppFlowPath -q
    } else {
        & $uvExe run --python $backendPython --with pytest --with pytest-playwright `
            pytest $e2eModelConfigPath $e2eAppFlowPath -q
    }
    if ($LASTEXITCODE -ne 0) { throw "E2E test command failed with exit code $LASTEXITCODE" }

    Write-Host "[DONE] Full model-config + app-flow regression suite passed."
}
finally {
