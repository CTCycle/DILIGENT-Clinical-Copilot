...
def get_active_brave_keys(api_context: APIRequestContext) -> list[dict]:
    ...
    if not isinstance(payload, list):
        return []
    return [
        row for row in payload if isinstance(row, dict) and row.get("is_active") is True
    ]
...
def test_research_returns_503_when_key_missing(api_context: APIRequestContext) -> None:
    if get_active_brave_keys(api_context):
        pytest.skip(
            "An active Brave Search access key is configured; missing-key path not applicable."
        )

    response = api_context.post(
        "/api/research", data={"question": "acetaminophen DILI evidence"}
    )
    assert response.status == 503
    assert "No active Brave Search access key" in response.text()

    alias_response = api_context.post(
        "/api/research", data={"question": "acetaminophen DILI evidence"}
    )
