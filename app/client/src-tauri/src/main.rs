...
fn render_startup_screen(
    ...
    let spinner_markup = if show_spinner {
        format!(
            "<div style='display:flex;align-items:center;gap:14px;margin:0 0 20px 0;'><div style='width:22px;height:22px;border:3px solid rgba(236,253,243,0.16);border-top-color:{accent};border-right-color:{accent};border-radius:50%;animation:diligent-spin 0.85s linear infinite;'></div><div style='font-size:12px;letter-spacing:0.16em;text-transform:uppercase;color:#bbf7d0;'>Launching services</div></div>"
        )
    } else {
        String::new()
    };
    ...
    let script = format!(
        "document.body.style.margin='0';document.body.style.background='radial-gradient(circle at top, #14532d 0%, #052e16 52%, #02140a 100%)';document.body.style.color='#ecfdf3';document.body.style.fontFamily='Segoe UI, sans-serif';document.body.innerHTML=\"<style>@keyframes diligent-spin{{from{{transform:rotate(0deg)}}to{{transform:rotate(360deg)}}}}</style><div style='min-height:100vh;display:flex;align-items:center;justify-content:center;padding:32px;box-sizing:border-box;'><div style='max-width:760px;width:100%;background:rgba(2,20,10,0.82);backdrop-filter:blur(12px);border:1px solid rgba(187,247,208,0.18);border-radius:22px;padding:32px 34px;box-shadow:0 28px 70px rgba(0,0,0,0.42);'>{spinner_markup}<h2 style='margin:0 0 12px 0;font-size:30px;font-weight:600;'>\"+ '{escaped_title}' +\"</h2><pre style='margin:0;white-space:pre-wrap;line-height:1.6;font-size:15px;color:#dcfce7;'>\"+ '{escaped_message}' +\"</pre></div></div>\";"
    );
...
fn render_startup_error(app_handle: &tauri::AppHandle, message: &str) {
    render_startup_screen(
        app_handle,
        "DILIGENT Desktop startup error",
        message,
        "#f87171",
        false,
    );
...
fn is_workspace_root(candidate: &Path) -> bool {
    candidate.join(APP_FOLDER).join("server").join("pyproject.toml").is_file()
        && candidate
            .join(APP_FOLDER)
            .join("server")
            .join("app.py")
...
fn directory_is_writable(path: &Path) -> bool {
    ...
    let probe_path = path.join(".diligent-write-probe");
...
fn should_sync_workspace(workspace_root: &Path, runtime_root: &Path) -> bool {
    ...
    let marker = runtime_root.join(".diligent-runtime-ready");
    if !marker.is_file() {
        return true;
    }
    ...
    for candidate in [
        workspace_root.join("app").join("server").join("pyproject.toml"),
        workspace_root.join("app").join("server").join("uv.lock"),
        workspace_root.join("app").join("server").join("uv.lock"),
...
fn ensure_runtime_lockfile(active_root: &Path, workspace_root: &Path) -> Result<(), String> {
    let runtime_lock = active_root.join("app").join("server").join("uv.lock");
    ...
    let source_candidates = vec![
        workspace_root.join("app").join("server").join("uv.lock"),
        workspace_root.join("app").join("server").join("uv.lock"),
        active_root.join("app").join("server").join("uv.lock"),
    ];
...
fn sync_workspace_payload(workspace_root: &Path, runtime_root: &Path) -> Result<(), String> {
    ...
    copy_file_if_exists(
        &workspace_root.join("app").join("server").join("pyproject.toml"),
        &runtime_root.join("app").join("server").join("pyproject.toml"),
    )?;
    ...
    fs::create_dir_all(runtime_root.join("app").join("resources").join("logs")).map_err(
        |error| {
            format!(
    ...
    fs::write(runtime_root.join(".diligent-runtime-ready"), b"ready").map_err(|error| {
        format!(
...
fn stage_runtime_lockfile(_active_root: &Path) -> Result<(), String> {
    Ok(())
}
...
fn spawn_backend(app_handle: &tauri::AppHandle, state: &BackendChildState) -> Result<(), String> {
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        let project_dir = active_root.join("app");
        ...
        let venv_dir = project_dir.join("server").join(".venv");
