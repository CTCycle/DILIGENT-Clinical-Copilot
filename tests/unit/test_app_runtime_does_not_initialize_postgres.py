...
def test_fastapi_app_import_does_not_trigger_database_initializer(monkeypatch) -> None:  # type: ignore[no-untyped-def]
    ...
    def _fake_ensure_postgres_database(_settings):  # type: ignore[no-untyped-def]
        ...
        raise AssertionError("PostgreSQL initializer should never run during app runtime import")

    monkeypatch.setattr(
        "DILIGENT.server.repositories.database.initializer.ensure_postgres_database",
        _fake_ensure_postgres_database,
    )

    sys.modules.pop("DILIGENT.server.app", None)
    importlib.import_module("DILIGENT.server.app")
