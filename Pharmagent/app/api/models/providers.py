...
class OllamaClient:
    ...
    async def unload_model(self, name: str) -> None:
        payload = {"model": name, "messages": [], "keep_alive": 0}
        try:
            resp = await self._client.post("/api/chat", json=payload)
        except httpx.TimeoutException as e:
            raise OllamaTimeout(f"Timed out unloading model '{name}'") from e
        except httpx.RequestError as e:  # noqa: PERF203 - normalize errors
            raise OllamaError(f"Failed to unload model '{name}': {e}") from e

        self._raise_for_status(resp)

    # -------------------------------------------------------------------------
    async def unload_models(self, names: list[str]) -> tuple[list[str], dict[str, str]]:
        unloaded: list[str] = []
