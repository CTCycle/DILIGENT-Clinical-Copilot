...
![DILIGENT Clinical Copilot system overview (v3.2.0)](assets/figures/clinical-copilot-overview-v3.2.0.png)
...
On a fresh checkout, select option 2 first to install or update dependencies and build the frontend, then select option 1 to launch the application. Use option 3 when the frontend needs to be rebuilt without synchronizing the backend environment. If option 1 detects missing or unusable dependencies or frontend output, it performs the same recovery build before launching.
